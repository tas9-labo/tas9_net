...
- ロゴ・ツールのアイコンは simple-icons（各社の公式ロゴを忠実にトレースしたベクター集・CC0）のパスをインライン展開。
  各ロゴの権利は各社のもの＝**色を変えない（ここでは単色）・形を変えない・提携を装わない**の範囲で「リンク先／使用ツールの表示」として使う。
  3ds Max と MotionBuilder は単体アイコンが無いので Autodesk のロゴ＋ラベルで示す（2026-09-15）。
