...
> **宿題（片付けたら消す）**
> 1. NFC タグを**ロック**（rune のメニューから。iPhone / Android の実機テストは 2026-09-15 に完了済み。ロックは元に戻せない）
> 2. `https://tas9.net/`（www 無し）の証明書が GitHub で発行されたか確認（`curl -sI https://tas9.net/` が 301 なら完了）
> 3. 名刺用紙（エーワン 51002）が届いたら普通紙で位置合わせ → 本番印刷
