...
- **3回目（2026-09-24・約20分）**：notes.tas9.net で効いた「カスタムドメインを外して付け直す」を apex にも試した。
  DNS（A×4・AAAA 無し・CAA 無し）と ACME の経路（`http://tas9.net/.well-known/acme-challenge/` が GitHub の 404）は正常なのに、
  証明書の状態は「new」のまま進まず。www→apex 転送の先に証明書が無い危険な状態になるので www に戻した（www は無事）。
  **結論：この apex には GitHub Pages が証明書を出さない（原因は GitHub 側・3回とも同じ）。4回目はやらない。**
  直すなら Cloudflare 等を前に置いて apex の TLS を肩代わりさせる道しかなく、DNS の管理先が変わる（Workspace のメール設定に触る）ため見送り。
...
>    配信中の証明書は **2026-12-13 まで有効**なので今は問題なし。同日に「外して付け直す」を1回試したが1時間では出ず。
>    **10月中に一度 `gh api repos/tas9-labo/tas9_net/pages` で `https_certificate.state` を確認**し、まだ new なら再度「外して付け直す」。
>    11月末までに approved にならなければ GitHub サポートへ（期限切れになると www に警告が出る）。approved になったらこの項目を消す。
> 1. **`https://tas9.net`（www 無し・https 直打ち）の警告は「直さない」と決定**（2026-09-16 本人・2026-09-24 に3回目を試して不発＝決定を維持）。名刺・タグ・QR・vCard はすべて www で正常、手打ちの https 直打ちのみの問題で実用上ゼロ。**今後この件を持ち出さない・深追いしない**
